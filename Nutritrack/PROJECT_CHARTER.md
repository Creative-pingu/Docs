...
1. **Offline-First**: All core features work without connectivity
2. **Local-Only Data Storage**: System does NOT collect any user data; everything is stored locally on-device
3. **No Telemetry**: No tracking, analytics, or external data transmission
4. **iPhone 16e Optimized**: Primary platform focus with Safari compatibility
...
- [x] No user data collected or transmitted without explicit action
...
- **User data collection/telemetry** (explicitly excluded)
- Any form of tracking or analytics
...
- **Privacy**: User data is private and local-only; no external collection
...
| **Local-Only Data** | NO user data collected; everything stored locally on-device |
...
| **No Backend Maintenance** | Only Cloudflare Worker for credential/CORS handling (no database, no server maintenance) |
...
| **Storage** | localStorage (5MB cap) | On-device data persistence (local-only) |
| **Food Database** | External foods.json (~342KB) | Nutritional data for 830 foods |
| **Sync** | Cloudflare Worker -> Notion API | Read-only recipe synchronization (user-initiated) |
...
- All user data (logs, goals, profile, custom foods, recipes) stored exclusively in localStorage
- No data transmitted to any external service without explicit user action
- Notion sync is user-initiated and read-only
- No telemetry, tracking, or analytics of any kind
- Cloudflare Worker only handles Notion API proxy requests (no user data storage)
...
4. **Review** verifies brief against current code state AND checks implementation quality, passes, and security
...
**Matrix Cycle**:
1. Perform the action (e.g., log an entry, change a profile field)
...
| **New dependencies** | Require Architect decision (not allowed as inline fixes) |
...
| **Privacy** | ZERO TOLERANCE for unauthorized data transmission |
...
| **Privacy Model** | Local-only, no data collection | Core principle | Active |
...
- 2026-08-14: Updated with core focus on local-only data storage and no user data collection
- 2026-08-14: Fixed decision-making process order (Review now after Dev steps)
